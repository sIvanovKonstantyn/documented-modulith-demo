...
public class ExtendedDocumenter extends Documenter {
    ...
    private void writeConfigurationDocumentation() {
        ...
        try (Writer writer = new FileWriter(file.toFile());
             BufferedReader br = new BufferedReader(new InputStreamReader(stream))) {//TODO support other types
            var documentationText = br.lines()
                .map(s -> {
                    if (s.startsWith("#")) {
                        return "_" + s.replace("#", "").trim() + "_";
                    } else {
                        var parts = s.split("=", 2);
                        if (parts.length < 2) {
                            return s;
    ...
    public void moveToFolder(String destinationDir) {
        ...
        Stream.concat(
            ...
            modules.stream()
                .map(
                    m-> Paths.get(getTargetFileName(m.getName()))
                )
        )
            .map(Path::toFile)
            .forEach(
                file -> {
                    try {
                        FileCopyUtils.copy(file, Paths.get(destinationDir, file.getName()).toFile());
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    }
                }
